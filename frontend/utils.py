...
def post_courses(course_data: list):
    ...
    try:
        ...
        st.error("Connection Error: Could not connect to the FastAPI backend. Please ensure the backend is running.")
        return None
    except requests.exceptions.HTTPError as e:
        st.error(f"HTTP Error: {e.response.status_code} - {e.response.json().get('detail', 'Unknown error')}")
        return None
    ...
    try:
        ...
        for course_obj in parsed_data:
            if not isinstance(course_obj, dict):
                return None, f"Each item in the JSON array must be a JSON object, but found: {type(course_obj).__name__}."
            
            # Create a new dictionary with only the expected fields
            cleaned_course = {
                key: value for key, value in course_obj.items() if key in expected_fields
            }
            cleaned_courses.append(cleaned_course)
            
        return cleaned_courses, None
    except json.JSONDecodeError as e:
        return None, f"Invalid JSON format. Please check syntax (e.g., missing commas, brackets, or quotes). Error: {e}"
    except Exception as e:
        return None, f"An unexpected error occurred during JSON parsing or cleaning: {e}"
