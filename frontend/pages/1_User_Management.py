    ...
    with st.form("registration_form"):
        ...
        if register_button:
            if new_password != confirm_password:
                ...
                st.error(
                    "Password too short! Needs at least 4 characters for proper safeguarding."
                )
            else:
                ...
                if response and response.status_code == 200:
                    st.success(
                        "Welcome, explorer! Your account is ready. Let the insights flow!"
                    )
                    st.balloons()
                    ...
                    try:
                        ...
                        if "detail" in response_json:
                            ...
                            error_detail = str(
                                response_json
                            )  # Fallback to full JSON if no detail/message
        ...
        if login_submit:
            if login_user_identifier and login_password:
                with st.spinner("Charting your course..."):
                    ...
                    if token_response:
                        st.success(
                            "Course charted! You're back in the data flow. Enjoy your journey."
                        )
                        ...
                        st.session_state["logged_in"] = (
                            True  # Indicate user is logged in
                        )
                        st.session_state["user_identifier"] = login_user_identifier
                        st.switch_page("pages/2_Ingest_Course_Data.py")
            else:
                st.error(
                    "Coordinates missing! Please provide both your User ID and Secret Code."
                )
