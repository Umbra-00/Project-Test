...
if not st.session_state.logged_in:
    st.info("Guest mode: Your browsing is temporary. Registered users' insights are saved. Ready to explore?")
...
if st.button("Fetch All Courses"):
    ...
    if courses:
        ...
        for i, course in enumerate(courses):
            ...
            st.write(f"**Price:** {course.get('price', 'N/A')} {course.get('currency', 'N/A')}")
...
selected_course_url = st.text_input("Enter Course URL", "http://example.com/fastapi-essentials")
...
if st.session_state.get("logged_in"):
    ...
    course_url = st.text_input("Enter a course URL you liked for recommendations", "http://example.com/fastapi-essentials")
    num_recs = st.number_input("Number of recommendations", min_value=1, max_value=10, value=3)
    if st.button("Get Recommendations"):
        from frontend.utils import get_recommendations
        recs = get_recommendations(user_id=user_id, course_history_urls=[course_url], num_recommendations=num_recs)
        if recs:
